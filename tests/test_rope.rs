...
fn rope_slice() {
    ...
    let rope = Rope::from(TEST_BYTES_1);
    assert_eq!(TEST_BYTES_1.len(), rope.len());

    rope.buf().copy_to(&mut dst);
    assert_eq!(dst, TEST_BYTES_1);

    let left = rope.slice_to(250);
...
#[test]
fn rope_concat_two_parts() {
    let mut dst = vec![];

    let left = Rope::from(TEST_BYTES_1);
    let right = Rope::from(TEST_BYTES_2);
...
#[test]
fn rope_equality() {
    let a = Rope::from(&b"Mary had a little lamb, its fleece was white as snow; "[..])
        .concat(Rope::from(&b"And everywhere that Mary went, the lamb was sure to go."[..]));

    let b = Rope::from(&b"Mary had a little lamb, "[..])
        .concat(Rope::from(&b"its fleece was white as snow; "[..]))
        .concat(
            Rope::from(&b"And everywhere that Mary went, "[..])
                .concat(Rope::from(&b"the lamb was sure to go."[..])));
...
fn rope_inequality() {
    let a = Rope::from(&b"Mary had a little lamb, its fleece was white as snow; "[..])
        .concat(Rope::from(&b"And everyhere that Mary went, the lamb was sure to go."[..]));
